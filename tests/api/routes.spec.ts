...
describe('API routes', () => {
  it('GET /api/timeline returns data and handles errors', async () => {
    ...
    const okRes = await okServer.request
      .get('/api/timeline?days=2&tz_offset=120')
      .expect(200);
  ...
  it('POST /api/goals validates input and redirects', async () => {
    ...
    const badRes = await badServer.request
      .post('/api/goals')
      .type('form')
      .send({ title: '' })
      .expect(307);
  ...
  it('POST /api/goals returns error redirect when creation fails', async () => {
    ...
    await mockCloudflare(env);
    const goalsModule = await import('@/db/goals');
    vi.spyOn(goalsModule, 'createGoal').mockRejectedValueOnce(
      new Error('fail')
    );
    const { POST } = await import('@/app/api/goals/route');
  ...
  it('POST /api/goals/[id]/target updates goal target or redirects on missing id', async () => {
    ...
    const missingServer = createRouteTester(POST, { id: '0' });
    const missingRes = await missingServer.request
      .post('/api/goals/0/target')
      .type('form')
      .send({ dailyTargetCount: 3 });
  ...
  it('POST /api/timeline/notes validates content and stores note', async () => {
    ...
    const emptyRes = await emptyServer.request
  ...
  it('DELETE/PATCH /api/goals/[id] cover CRUD flows', async () => {
    ...
    const missingRedirectRes =
      await missingRedirect.request.post('/api/goals/0');
    expect(missingRedirectRes.headers.location).toContain('missing_goal_id');
    ...
    const failDeleteSpy = vi
      .spyOn(goalsModule, 'deleteGoal')
      .mockRejectedValueOnce(new Error('boom'));
    const failDeleteServer = createRouteTester(POST, { id: String(goal.id) });
    const failDeleteRes = await failDeleteServer.request.post(
      `/api/goals/${goal.id}`
    );
    ...
    vi.spyOn(goalsModule, 'deleteGoal').mockRejectedValueOnce(
      new Error('boom-json')
    );
    const failJsonRes = await failJsonServer.request
      .delete(`/api/goals/${goal.id}`)
      .set('accept', 'application/json');
    ...
    const missingPatchRes = await missingPatch.request
      .patch('/api/goals/0')
      .send({})
    ...
    vi.spyOn(goalsModule, 'updateGoal').mockRejectedValueOnce(
  ...
  it('DELETE/POST /api/timeline/notes/[id] remove notes or validate id', async () => {
    ...
    const goalsModule = await import('@/db/goals');
    const { POST, DELETE } =
      await import('@/app/api/timeline/notes/[id]/route');

    const missingServer = createRouteTester(DELETE, { id: '0' });
    const missingRes = await missingServer.request.delete(
      '/api/timeline/notes/0'
    );
    ...
    const postRes = await postServer.request.post(
      `/api/timeline/notes/${note.id}`
    );
    ...
    vi.spyOn(goalsModule, 'deleteTimelineNote').mockRejectedValueOnce(
      new Error('fail')
    );
    const errorServer = createRouteTester(DELETE, { id: String(note.id) });
    const errorRes = await errorServer.request.delete(
      `/api/timeline/notes/${note.id}`
    );
    expect(errorRes.status).toBe(500);
    await errorServer.close();
